...
const totalPages = Math.ceil(history.length / itemsPerPage);
    ...
    const handleApiError = (err) => {
        if (err.message.includes('Unauthorized')) setIsLoggedIn(false);
    ...
    const fetchData = useCallback(async (isSilent = false) => {
        ...
        try {
            ...
            const bRes = await fetch(`${API_BASE}/balance?${cacheBuster}`).then(r => {
            ...
            const historyUrl = `${API_BASE}/history?start=${startDate}&end=${endDate}${cacheBuster}`;
...
setHistory(rawActivities); // แสดงทุกรายการโดยไม่ต้อง filter
    ...
    useEffect(() => {
        if (p2pForm.phone.length === 10 && isLoggedIn) {
            ...
            fetch(`${API_BASE}/recipient/${p2pForm.phone}`)
                .then(r => {
                    if(r.status === 401) throw new Error("Unauthorized");
                    return r.json();
                })
    ...
    const handleP2P = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`${API_BASE}/transfer/p2p`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleBankTransfer = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`${API_BASE}/transfer/bank`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            <div className="max-w-5xl mx-auto">
                ...
                <div className="flex justify-between items-center mb-6 px-2">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <button onClick={() => setIsLoggedIn(false)} className="text-slate-400 hover:text-red-500 transition-colors flex items-center text-xs font-bold">
